...
class EC_Customer:
    ...
    def enviar_mensaje(self):
        if not self.solicitudes_enviada:
            if self.solicitudes_pendientes:
                self.solicitud_enviada = True
                solicitud = self.solicitudes_pendientes.pop(0)
                ...
                self.producer.send('cliente_solicitud', value=json.dumps(mensaje).encode())
